...
export class MediaOnlyForum extends ThreadFeatureBase {
  ...
  public async action(channel: AnyThreadChannel) {
    ...
    // Kirim reminder
    await post.author.send(`Halo ${post.author.displayName} 😊, Untuk membuat postingan di channel ${channel.parent.name} kamu perlu mnyertakan foto atau video ya. Maaf, postingan kamu sebelumnya terpaksa aku hapus ya 😢.`)
  }
}